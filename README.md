...
9.  **模块G：企业级收件人分群管理 (Enterprise Recipient Segmentation)**
    ...
    - [x] **核心性能优化：攻克深分页难题**
        - **问题**: 当收件人分群结果超过 10,000 人时，依赖传统的 `from` + `size` 分页会触发 Elasticsearch 的 `max_result_window` 保护机制，导致任务失败。
        - **解决方案**: 将 Elasticsearch 的查询机制从 `from` + `size` 全面升级为**更高性能的 `search_after` 模式**。这种基于游标的深分页技术，使得系统无论处理十万还是百万级收件人，都能保持稳定、高效的内存占用和查询性能，彻底解决了海量数据下的分页瓶颈。
...
10. **模块H：用户认证与授权 (User Auth & Permissions)**
    ...
    - [x] **性能再优化：分发器效率提升 (Further Performance Tuning)**
        - **问题**: 在处理包含数万收件人的大型任务时，分发器（`TaskDispatcherService`）自身也存在性能瓶颈。它需要将一个大任务拆分成多个"页面"进行处理，但原始实现会在每个页面循环中重复查询数据库。
        - **解决方案**:
            1.  **消除循环内 N+1 查询**: 将分群规则（`RecipientGroup`）和邮件模板（`Template`）的查询操作，从分页循环**内部提升到外部**。这意味着无论任务需要分多少页，这些不变的核心数据都只会被查询一次，根除了分发阶段的 N+1 问题。
            2.  **数据库记录批量插入 (Batch Inserts)**: 在每个分页周期中，分发器不再是为每个收件人创建一条记录，而是将整个页面的数千条 `EmailSendRecord` 在一次数据库交互中**批量创建**，大幅降低了数据库写操作的压力和网络往返次数。
