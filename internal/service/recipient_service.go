...
func NewRecipientService(repo repository.RecipientRepository, queueSvc queue.QueueService) RecipientService {
...
func (s *recipientService) CreateRecipient(email string, firstName, lastName *string, metadata map[string]interface{}) (*model.Recipient, error) {
	...
	// Enqueue for ES sync
	if err := s.queue.Enqueue(context.Background(), queue.RecipientSyncQueue, strconv.FormatInt(recipient.ID, 10)); err != nil {
		log.Printf("CRITICAL: Recipient %d created in DB but failed to enqueue for ES sync: %v", recipient.ID, err)
		// Don't fail the API call, but log it as a critical issue.
	}
...
func (s *recipientService) UpdateRecipient(id uint, email *string, firstName, lastName *string, status *string, metadata map[string]interface{}) (*model.Recipient, error) {
	...
	// Enqueue for ES sync
	if err := s.queue.Enqueue(context.Background(), queue.RecipientSyncQueue, strconv.FormatInt(recipient.ID, 10)); err != nil {
		log.Printf("CRITICAL: Recipient %d updated in DB but failed to enqueue for ES sync: %v", recipient.ID, err)
	}
...
func (s *recipientService) DeleteRecipient(id uint) error {
	...
	// Here you might also enqueue a delete operation for ES.
	// For simplicity, we'll focus on create/update for now.
	return nil
}
