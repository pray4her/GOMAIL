...
**传统分页参数**: `page` (页码), `pageSize` (每页数量)
**Search_After分页参数**: `search_after` (Base64编码的分页标记), `page_size` (每页数量)
**筛选参数**: `name` (姓名模糊搜索), `email` (邮箱精确匹配), `metadata.<key>` (元数据字段匹配)

**注意**: 当请求深层页面时(页码 × 每页数量 ≥ 10000)，需要使用search_after分页。
...
**1. 传统分页 (Traditional Pagination)**
适用于大多数列表接口，使用 `page` 和 `pageSize` 查询参数。
...
**2. Search_After分页 (Search_After Pagination)**
适用于深度分页场景（如收件人列表），使用 `search_after` 和 `page_size` 查询参数。
